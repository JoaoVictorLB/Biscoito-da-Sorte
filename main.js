...
const POSITIONS_IN_ARRAY = TOTAL_MESSAGES - 1;
const PLAY_BUTTON = document.querySelector("#play");
const TRY_AGAIN_BUTTON = document.querySelector("#play-again");
const FIRST_PAGE_CLASS_LIST = document.querySelector(".first-view").classList;
const SECOND_PAGE_CLASS_LIST = document.querySelector(".second-view").classList;

PLAY_BUTTON.addEventListener('click', pickUpMessage);
TRY_AGAIN_BUTTON.addEventListener('click', switchScreens);

function pickUpMessage(){
    let message = drawMessage();
    document.querySelector(".second-view p").innerText = message;
    switchScreens();
}

function drawMessage(){
    let randomNumber = Math.floor(Math.random() * POSITIONS_IN_ARRAY);
    return MESSAGES[randomNumber];
}

function switchScreens(){
    FIRST_PAGE_CLASS_LIST.toggle("hide");
    SECOND_PAGE_CLASS_LIST.toggle("hide");
}